<x-app-layout title="Quote Attachments" bodyClass="page-my-quotes">
    <main>
      <div>
        <div class="container">
          ...
          <div class="quote-attachments-wrapper">
            <form action="{{ route('quotes.updateAttachments', $quote) }}"
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="quote-form-attachments">
                        @if(Str::startsWith($attachment->mime_type, 'image/'))
                          <img src="{{ Storage::url($attachment->path) }}" alt="" class="quote-form-attachment-preview" />
                        @elseif(Str::startsWith($attachment->mime_type, 'video/'))
                          <video src="{{ Storage::url($attachment->path) }}" class="quote-form-attachment-preview" controls muted></video>
                        @elseif(Str::startsWith($attachment->mime_type, 'application/pdf'))
                          <div class="document-preview">
                              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="currentColor" class="bi bi-file-pdf" viewBox="0 0 16 16">
                    ...
                    <tr>
                      <td colspan="3" class="text-center p-large">
                        No attachments were uploaded for this quote.
                      </td>
                    </tr>
                    @endforelse
                  </tbody>
              ...
              <div class="p-medium">
                <div class="flex justify-end gap-3">
                  <a href="{{ route('quotes.edit', $quote) }}" class="btn btn-default" style="margin: 0.1rem">Back to Quote</a>
                  <button type="submit" class="btn btn-primary" style="margin: 0.1rem">Save Changes</button>
                </div>
              </div>
            </form>
            <form action="{{ route('quotes.addAttachments', $quote) }}"
            ...
            @csrf
              <div class="form-attachment-upload">
                ...
                <input id="quoteFormAttachmentUpload" type="file" name="attachments[]" multiple accept="image/*,video/*,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain" />
              </div>
  ...
  @vite(['resources/js/quotes-attachments.js'])